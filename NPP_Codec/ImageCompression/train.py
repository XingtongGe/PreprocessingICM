...
if __name__ == "__main__":
    ...
    net = model.cuda()
    logger.info(net)
    # net = torch.nn.DataParallel(net, list(range(gpu_num))) # 使用多个gpu进行训练
    parameters = net.parameters()
    global test_loader
    test_dataset = TestKodakDataset(data_dir='/data4/luguo/datasets/kodak')
    test_loader = DataLoader(dataset=test_dataset, shuffle=False, batch_size=4, pin_memory=True, num_workers=1)
    if args.test:
        testKodak(global_step)
        exit(-1)
    ...
    train_data_dir = '/data4/luguo/datasets/Flick_patch'
    ...
    train_loader = DataLoader(dataset=train_dataset,
                              ...
                              pin_memory=True,
                              num_workers=2)
    # 这里可能会是预加载模型  所以steps_epoch需要进行计算   tot_epoch类似无穷大的使用  规定一个边界
    steps_epoch = global_step // (len(train_dataset) // (batch_size))# * gpu_num))     # //为取整除运算
    save_model(model, global_step, save_path)
